function [opt, RMSE_Evolution] = Synthetic_Data_GBPRBM_Train(varargin)
...
if length(varargin)==2
    ...
    load(fullfile('Synth_Data', 'Synth_Data_Train_V=4.mat'));
end
...
for x = 1:N_Epochs
    ...
    % Train the GBPRBM model using Contrastive Divergence
    opt = GBPRBM_Train(trainData, opt);
    RMSE_Evolution((x-1)*M+1:x*M) = opt.RMSE_Evolution;
end
opt.DirSave = sprintf('Synth_Data_GBPRBM_Model_V=%d', V);
if exist(opt.DirSave,'dir') ~=7
   mkdir(opt.DirSave);
end
% A *.mat file to save the GBPRBM model
FileName = fullfile(opt.DirSave, sprintf('Synth_Data_GBPRBM_Model,H=%i.mat', opt.H));
save(FileName, '-struct', 'opt');
save(FileName, 'RMSE_Evolution', '-append');

h_fig = figure('Name', sprintf('Synth_Data GBPRBM, H=%d: Evolution of RMSE (train)', opt.H), ...
               'NumberTitle', 'Off');
