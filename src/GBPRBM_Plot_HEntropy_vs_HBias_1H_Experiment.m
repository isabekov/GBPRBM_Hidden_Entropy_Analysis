function opt = GBPRBM_Plot_HEntropy_vs_HBias_1H_Experiment(varargin)
...
%             GBPRBM_Plot_HEntropy_vs_HBias_1H_Experiment
% Or pass a structure with parameters, look down below for the details
%             GBPRBM_Plot_HEntropy_vs_HBias_1H_Experiment(opt)
...
if exist('h_axes','var')==1
   ...
   figure('Name', 'Hidden entropy vs. Hidden bias for H=1 (Experiment)', 'NumberTitle', 'Off');
end
...
b_h = linspace(b_h_lim(1), b_h_lim(2), N_Samples(1))';

Nm = zeros(2^H,N_Samples);
p_h = zeros(2^H,N_Samples);
Zn  = zeros(1,N_Samples);
% For every configuration of the hidden layer  (2^H different configurations)
idx = 1;
for h= [-1 +1]
   B = exp(1/2*(2*b_v + W*h)'*Sinv*W*h);
   Nm(idx,:) = B*exp(b_h).^h;
   Zn = Zn + Nm(idx,:);
   idx = idx + 1;
end
...
Entropy = zeros(1, N_Samples);
for k = 1:2^H
    Entropy = Entropy - p_h(k,:).*log2(p_h(k,:));
end
...
set(gca, 'YDir', 'normal');
xlabel('b_h(1)');
opt.h_title =  title('Hidden entropy [bits] (experiment)');
